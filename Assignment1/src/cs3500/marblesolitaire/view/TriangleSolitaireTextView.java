...
public class TriangleSolitaireTextView implements MarbleSolitaireView {
  ...
  public String toString() {
    ...
    for (int i = 0; i < this.model.getBoardSize(); i++) {
      boolean flag = false;
      pad = " ".repeat((i % 2) - ((this.model.getBoardSize() + 1) % 2) + (this.model.getBoardSize() - i - (i % 2)) / 2 * 2);
      Utils.write(ap, pad);
      for (int j = 0; j < this.model.getBoardSize(); j++) {
        Utils.write(ap, slotStateToString(this.model.getSlotAt(i, j)));
        if (!flag && !this.model.getSlotAt(i, j).equals(MarbleSolitaireModelState.SlotState.Invalid)) {
          flag = true;
        }
        if (flag && j < this.model.getBoardSize() - 1
                && this.model.getSlotAt(i, j + 1).equals(MarbleSolitaireModelState.SlotState.Invalid)
